...
public class MainActivity extends AppCompatActivity {
    ...
    private void allCalculations() {
        if (!Double.isNaN(firstValue)) {
            String inputText = inputDisplay.getText().toString().trim();

            // Check if input is not empty before attempting to parse
            if (!inputText.isEmpty()) {
                secondValue = Double.parseDouble(inputText);
                inputDisplay.setText(null);
            } else {
            ...
            // Check if more than one operator is used
            if (inputText.contains("+") ||
                    inputText.contains("-") ||
                    inputText.contains("x") ||
                    inputText.contains("/") ||
                    inputText.contains("%")) {
                showToast("Error: More than one operator used");
                resetCalculator();
            }
        } else {
            try {
